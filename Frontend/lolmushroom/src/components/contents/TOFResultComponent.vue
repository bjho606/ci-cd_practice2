<script setup>
  ...
  const totalParticipants = computed(() => {
    return Object.values(choiceAnswerMatrix[myNickName]).reduce((total, nicknames) => total + nicknames.length, 0);
  });

  const correctRate = choiceAnswerMatrix[targetNickName][answer.value].length / totalParticipants.value
</script>
        ...
        <v-card v-for="i in 4" :key="i">
          ...
          <v-expand-transition>
            <v-card-text v-if="expanded[i]">
              <ul>
                <li v-for="nickname in choiceAnswerMatrix[targetNickName][i]" :key="nickname">{{ nickname }}</li>
              </ul>
            </v-card-text>
          </v-expand-transition>
